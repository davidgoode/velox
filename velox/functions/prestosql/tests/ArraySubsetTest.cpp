...
class ArraySubsetFuzzerTest : public test::FunctionBaseTest {
 protected:
  ...
  static SelectivityVector getNonNullRows(const RowVectorPtr& data) {
    ...
    // Use isNullAt() for reliable null checking across all vector encodings.
    // rawNulls() can be unreliable with complex vector types from the fuzzer.
    for (vector_size_t row = 0; row < data->size(); ++row) {
      if (inputArray->isNullAt(row) || indices->isNullAt(row)) {
